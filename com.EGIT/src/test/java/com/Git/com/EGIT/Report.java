...
//	static Properties log4jProperties = PropertiesFileReader.getInstance().readProperties("log4j.properties");
	...
	public void logMessage(WebDriver driver, BufferedWriter writer, String ScreenshotName, String stepDesc, String expectedMsg, String actualMsg, String status)
	{
		...
		{
			log(driver, writer, ScreenshotName, String.valueOf(stepDesc) + ";" + expectedMsg + ";" + actualMsg, "Passed");
			infoToLog4j("Status : " + status + " :: " + actualMsg);
		}
		else
		{
			log(driver, writer, ScreenshotName, String.valueOf(stepDesc) + ";" + expectedMsg + ";" + actualMsg, "Failed");
	...
	public void addLogToFile(WebDriver driver, BufferedWriter writer, String ScreenshotName, int stepNum, String stepDesc, String expectedMsg, String actualMsg, String status)
	{
		try
		{
			...
			//System.out.println("PassedStepScreenshotRequired : "+PropertiesFileReader.getProperty("PassedStepScreenshotRequired"));
			//System.out.println("FailedStepScreenshotRequired : "+PropertiesFileReader.getProperty("FailedStepScreenshotRequired"));
			writer.write("<TR>"
			...
			{
				String imgName = takeScreenShotPASSANDFAIL(driver, stepDesc, stepNum);

				System.out.println("Taken screenshot : "+imgName);
				failedcount++;
				writer.write("<TD class='tsgen danger' width=50px align=center>"
						+ "<a target='_blank' href='../images/"+imgName+"'>Failed</a>"
						+ "<img class='screen' src = ''></TD>"+ " </TR>");
				setErrorMsg(actualMsg);
			}
			
					/*+ " <TD class='tsgen success success' width=50px align=center>Passed<img class='screen' src = ''></TD>"
					+ " </TR>");*/
			...
			/*if ("Passed".equalsIgnoreCase(status))
			{
				...
				writer.write("<TD class='tsgen' width=50px align=center><img class='screen' src = 'http://fdk-stage.cisco.com/content/dam/en/i/test/tick.png'></TD>");
			}
			else
			{
				writer.write("<TD class='tsgen' width=50px align=center><img class='screen' src = 'http://fdk-stage.cisco.com/content/dam/en/i/test/cross.png'></TD>");
				setErrorMsg(actualMsg);
				failedcount += 1;
			}
			if ("Passed".equalsIgnoreCase(status))
			{
				String PassedStepScreenshotRequired = PropertiesFileReader.getProperty("PassedStepScreenshotRequired");
				if ("Yes".equalsIgnoreCase(PassedStepScreenshotRequired))
				{
					String imgName = takeScreenShotPASSANDFAIL(driver, stepDesc, stepNum);
					screenShotHyperLinkNew(writer, imgName);
				}
				else
				{
			...
			{
				String failedStepScreenshotRequired = PropertiesFileReader.getProperty("FailedStepScreenshotRequired");
				if ("Yes".equalsIgnoreCase(failedStepScreenshotRequired))
				{
					String imgName = takeScreenShotPASSANDFAIL(driver, stepDesc, stepNum);
					screenShotHyperLinkNew(writer, imgName);
				}
	...
	public String takeScreenShotPASSANDFAIL(WebDriver driver, String stepDesc, int stepNum)
	{
		String imgName = "";
		String imgpath = "";
		File scrFile = null;
		try
		{
			if (driver != null)
			{
				...
				imgpath = String.valueOf(getReportDirPath()) + System.getProperty("file.separator") + "Reports(" + getDateTime() + ")" + System.getProperty("file.separator") + "images" + System.getProperty("file.separator");
				imgName = UUID.randomUUID().toString() + ".png";
				setFailedScreenshotPath(imgpath + imgName);
				setFailedHtmlPath(htmlPath);
				FileUtils.copyFile(scrFile, new File(String.valueOf(imgpath) + imgName));
				imageCounter += 1;
			}
		}
		catch (Exception e)
